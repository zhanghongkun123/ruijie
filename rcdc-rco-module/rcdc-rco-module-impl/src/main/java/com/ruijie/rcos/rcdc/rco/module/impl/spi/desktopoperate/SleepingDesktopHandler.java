 ...
 */
@Service
public class SleepingDesktopHandler extends AbstractMessageHandlerTemplate<CbbDeskOperateNotifyRequest> {
    ...
    protected boolean isNeedHandleMessage(CbbDeskOperateNotifyRequest request) {
        return CbbCloudDeskOperateType.SLEEPING_DESK == request.getOperateType();
    }

    @Override
    protected void processMessage(CbbDeskOperateNotifyRequest request) throws Exception {
        uwsDockingAPI.notifyDesktopStateUpdate(request.getDeskId(), CbbCloudDeskState.SLEEPING);
        rcaHostAPI.notifyDesktopStateUpdate(request.getDeskId(), CbbCloudDeskState.SLEEPING);
    }
}
