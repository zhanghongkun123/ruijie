...
import static com.ruijie.rcos.rcdc.rco.module.impl.PublicTaskBusinessKey.RCDC_RCO_MASTER_IMAGE_COMPLETE_HANDLER;
 ...
 */
@Service
public class MasterCompleteStageStateHandler implements StateTaskHandle {
    ...
    public String getName() {
        return LocaleI18nResolver.resolve(RCDC_RCO_MASTER_IMAGE_COMPLETE_HANDLER);
    }
    ...
    public void registerStateTransition(StateTransitionRegistry registry) {
        ...
        registry.add(CompleteImageSyncFinishProcessor.class);
    }
    ...
    public ExceptionTranslator getDefaultDoExceptionTranslator() {
        return new AbstractGlobalExceptionTranslator() {
            ...
            protected ProcessResult dealwithException(StateProcessContext context, Exception ex, String errorMsg) throws Exception {
                MasterCompleteImageSyncRequest request = resolver.resolveDTO(context);
                CbbImageTemplateDetailDTO imageTemplateDetail = imageTemplateMgmtAPI.getImageTemplateDetail(request.getImageTemplateId());
                auditLogAPI.recordLog(BusinessKey.RCDC_RCO_MASTER_COMPLETE_ERROR, imageTemplateDetail.getImageName(),
                        errorMsg);
                throw ex;
            }
        };
