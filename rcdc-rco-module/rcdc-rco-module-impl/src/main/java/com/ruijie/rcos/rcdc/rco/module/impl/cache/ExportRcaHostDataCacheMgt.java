 ...
 */
@Service
public class ExportRcaHostDataCacheMgt {
    private static final Logger LOGGER = LoggerFactory.getLogger(ExportRcaHostDataCacheMgt.class);

    private static final Cache<String, ExportRcaHostFileInfoDTO> CACHE_MAP;
    ...
    public ExportRcaHostDataCacheMgt() {
     ...
     * @param state 缓存状态
     */
    public void updateState(String key, ExportRcaHostDataStateEnums state) {
        Assert.hasText(key, "key不能为空");
        Assert.notNull(state, "ExportCloudDesktopDataStateEnums不能为null");
        ExportRcaHostFileInfoDTO cache = CACHE_MAP.getIfPresent(key);
        if (Objects.isNull(cache)) {
            cache = new ExportRcaHostFileInfoDTO();
        }
     ...
     * @param cache 缓存
     */
    public void save(String key, ExportRcaHostFileInfoDTO cache) {
        Assert.hasText(key, "key不能为空");
    ...
    public void deleteCache(String key) {
        ...
        ExportRcaHostFileInfoDTO cache = getCache(key);
    ...
    private void clearOldCacheAndDelFile(String key, ExportRcaHostFileInfoDTO value) {
     ...
     */
    public ExportRcaHostFileInfoDTO getCache(String key) {
        Assert.hasText(key, "key不能为空");
