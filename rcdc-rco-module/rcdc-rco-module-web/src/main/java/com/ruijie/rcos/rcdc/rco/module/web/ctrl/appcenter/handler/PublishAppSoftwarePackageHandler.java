 ...
 * @author chenli
 */
public class PublishAppSoftwarePackageHandler extends AbstractSingleTaskHandler {

    protected static final Logger LOGGER = LoggerFactory.getLogger(PublishAppSoftwarePackageHandler.class);
    ...
    public PublishAppSoftwarePackageHandler(BatchTaskItem batchTaskItem,
    ...
    public BatchTaskItemResult processItem(BatchTaskItem item) throws BusinessException {
        ...
        try {
            cbbAppSoftwarePackageMgmtAPI.publishAppSoftwarePackage(appSoftwarePackageId);

            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.SUCCESS)
                    .msgKey(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_PUBLISH_TASK_SUCCESS)
                    .msgArgs(appSoftwarePackageDTO.getName())
                    .build();
        } catch (BusinessException e) {
            LOGGER.error("发布应用软件包出错", e);
            String errorMsg = e.getI18nMessage();
            auditLogAPI.recordLog(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_PUBLISH_FAIL_LOG,
                    appSoftwarePackageDTO.getName(), e.getI18nMessage());
            throw new BusinessException(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_PUBLISH_FAIL_LOG, e,
    ...
    public BatchTaskFinishResult onFinish(int successCount, int failCount) {
        if (failCount == 0) {
            return DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.SUCCESS)
                    .msgKey(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_PUBLISH_TASK_SUCCESS)
                    .msgArgs(new String[]{appSoftwarePackageDTO.getName()})
                    .build();
        } else {
            return DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.FAILURE)
                    .msgKey(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_PUBLISH_TASK_FAIL)
                    .msgArgs(new String[]{appSoftwarePackageDTO.getName()})
                    .build();
        }
    }
