 ...
 * @author zhangyichi
 */
public class DistributeSubTaskCancelTaskHandler extends AbstractBatchTaskHandler {
    ...
    public DistributeSubTaskCancelTaskHandler(Iterator<? extends BatchTaskItem> batchTaskItemIterator) {
    ...
    public BatchTaskItemResult processItem(BatchTaskItem batchTaskItem) throws BusinessException {
        ...
        try {
            fileDistributionTaskManageAPI.cancelSubTask(new IdRequest(subTaskId));
            auditLogAPI.recordLog(FileDistributionBusinessKey.RCDC_RCO_FILE_DISTRIBUTION_CANCEL_SUB_TASK_SUCCESS_LOG,
                    parentTaskName, targetName);
            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.SUCCESS)
                    .msgKey(FileDistributionBusinessKey.RCDC_RCO_FILE_DISTRIBUTION_CANCEL_SUB_TASK_ITEM_SUCCESS_DESC)
                    .msgArgs(parentTaskName, targetName)
                    .build();
        } catch (BusinessException e) {
            auditLogAPI.recordLog(FileDistributionBusinessKey.RCDC_RCO_FILE_DISTRIBUTION_CANCEL_SUB_TASK_FAIL_LOG,
                    parentTaskName, targetName, e.getI18nMessage());
            throw new BusinessException(FileDistributionBusinessKey.RCDC_RCO_FILE_DISTRIBUTION_CANCEL_SUB_TASK_ITEM_FAIL_DESC, e, parentTaskName,
    ...
    public BatchTaskFinishResult onFinish(int successCount, int failCount) {
        return BatchTaskUtils.buildBatchTaskFinishResult(
                successCount, failCount, FileDistributionBusinessKey.RCDC_RCO_FILE_DISTRIBUTION_CANCEL_SUB_TASK_RESULT);
    }
}
