 ...
 */
public class RcaPoolUnbindThirdPartyHostBatchTaskHandler extends AbstractBatchTaskHandler {
    ...
    public RcaPoolUnbindThirdPartyHostBatchTaskHandler(Iterator<? extends BatchTaskItem> iterator, BaseAuditLogAPI auditLogAPI,
    ...
    public BatchTaskItemResult processItem(BatchTaskItem batchTaskItem) throws BusinessException {
        ...
        try {
            ...
            rcaAppPoolAPI.unbindThirdPartyHost(appPoolId, Lists.newArrayList(hostId));

            auditLogAPI.recordLog(RcaBusinessKey.RCDC_RCA_APP_POOL_BATCH_UNBIND_HOST_TASK_ITEM_SUCCESS, appPoolName, hostName);
            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.SUCCESS)
                    .msgKey(RcaBusinessKey.RCDC_RCA_APP_POOL_BATCH_UNBIND_HOST_TASK_ITEM_SUCCESS)
                    .msgArgs(new String[] {appPoolName, hostName}).build();
        } catch (Exception e) {
            LOGGER.error("应用池[{}]移除第三方主机[{}]发生异常，ex :", appPoolName, hostName, e);
            String errorMsg = e.getMessage();
            if (e instanceof BusinessException) {
                errorMsg = ((BusinessException) e).getI18nMessage();
            }
            auditLogAPI.recordLog(RcaBusinessKey.RCDC_RCA_APP_POOL_BATCH_UNBIND_HOST_TASK_ITEM_FAIL, appPoolName, hostName, errorMsg);
            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.FAILURE)
                    .msgKey(RcaBusinessKey.RCDC_RCA_APP_POOL_BATCH_UNBIND_HOST_TASK_ITEM_FAIL)
    ...
    public BatchTaskFinishResult onFinish(int successCount, int failCount) {
        ...
        if (failCount == 0) {
            return DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.SUCCESS)
                    .msgKey(RcaBusinessKey.RCDC_RCA_APP_POOL_UNBIND_HOST_SUCCESS)
                    .msgArgs(new String[]{appPoolName})
                    .build();
        } else {
            return DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.FAILURE)
                    .msgKey(RcaBusinessKey.RCDC_RCA_APP_POOL_UNBIND_HOST_FAIL)
                    .msgArgs(new String[]{appPoolName})
                    .build();
        }
    }
