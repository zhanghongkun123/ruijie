...
/**
 * Description: 分页查询应用池关联的所有用户（用户组下的用户+分配的用户）
 ...
 * @author linke
 */
public class RcaAppGroupBindUserPageRequest extends UserPageSearchRequest {


    private static final String APP_GROUP_ID = "appGroupId";

    public RcaAppGroupBindUserPageRequest(PageWebRequest webRequest) {
        super(webRequest);
    }
    ...
    protected MatchEqual[] exactMatchConvert(ExactMatch[] exactMatchArr) {
        ...
        for (MatchEqual matchEqual : matchEqualArr) {
            if (APP_GROUP_ID.equals(matchEqual.getName())) {
                if (ArrayUtils.isNotEmpty(matchEqual.getValueArr())) {
                    appGroupId = UUID.fromString(matchEqual.getValueArr()[0].toString());
                }
                continue;
            }
