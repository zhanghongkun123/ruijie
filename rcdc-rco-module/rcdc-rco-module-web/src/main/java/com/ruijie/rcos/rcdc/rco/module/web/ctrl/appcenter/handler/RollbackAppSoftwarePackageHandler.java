 ...
 * @author chenli
 */
public class RollbackAppSoftwarePackageHandler extends AbstractSingleTaskHandler {

    protected static final Logger LOGGER = LoggerFactory.getLogger(RollbackAppSoftwarePackageHandler.class);
    ...
    public RollbackAppSoftwarePackageHandler(BatchTaskItem batchTaskItem, BaseAuditLogAPI auditLogAPI,
    ...
    public BatchTaskItemResult processItem(BatchTaskItem item) throws BusinessException {
        ...
        try {
            cbbAppSoftwarePackageMgmtAPI.rollbackAppSoftwarePackage(appSoftwarePackageId);
            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.SUCCESS)
                    .msgKey(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_ROLLBACK_TASK_SUCCESS)
					.msgArgs(appSoftwarePackageDTO.getName()).build();
        } catch (BusinessException e) {
            LOGGER.error("应用软件包[{}]版本回退出错", appSoftwarePackageId, e);
            String errorMsg = e.getI18nMessage();
            auditLogAPI.recordLog(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_ROLLBACK_FAIL_LOG, appSoftwarePackageDTO.getName(),
                    e.getI18nMessage());
            throw new BusinessException(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_ROLLBACK_FAIL_LOG, e,
    ...
    public BatchTaskFinishResult onFinish(int successCount, int failCount) {
        if (failCount == 0) {
            ...
            return DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.FAILURE)
                    .msgKey(AppCenterBusinessKey.RCDC_APP_CENTER_APP_SOFTWARE_PACKAGE_ROLLBACK_TASK_FAIL)
                    .msgArgs(new String[] {appSoftwarePackageDTO.getName()}).build();
        }
    }
}
