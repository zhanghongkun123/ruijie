 ...
 * @author liuwc
 */
public class RebootThirdPartRcaHostBatchTaskHandler extends AbstractBatchTaskHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(RebootThirdPartRcaHostBatchTaskHandler.class);

    private RcaHostAPI rcaHostAPI;

    private BaseAuditLogAPI auditLogAPI;

    public RebootThirdPartRcaHostBatchTaskHandler(Iterator<? extends BatchTaskItem> iterator, BaseAuditLogAPI auditLogAPI) {
        super(iterator);
    ...
    public BatchTaskItemResult processItem(BatchTaskItem taskItem) throws BusinessException {
        ...
        if (!ObjectUtils.isEmpty(rcaHostDTO) && rcaHostDTO.getStatus() == RcaEnum.HostStatus.OFFLINE) {
            LOGGER.error("应用主机[{}]重启失败，应用主机离线", hostId);
            ...
            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.FAILURE)
                    .msgKey(RcaBusinessKey.RCDC_RCA_HOST_THIRD_PARTY_REBOOT_OFFLINE).msgArgs(new String[]{hostName}).build();
        }

        try {
            rcaHostAPI.rebootThirdPartyHost(hostId);
            auditLogAPI.recordLog(RcaBusinessKey.RCDC_RCA_HOST_THIRD_PARTY_REBOOT_SUCCESS, hostName);
            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.SUCCESS)
                    .msgKey(RcaBusinessKey.RCDC_RCA_HOST_THIRD_PARTY_REBOOT_SUCCESS).msgArgs(new String[]{hostName}).build();
        } catch (BusinessException e) {
            LOGGER.error("通知应用主机应用主机重启,主机id={}, 名称={}", hostId, hostName);
            auditLogAPI.recordLog(RcaBusinessKey.RCDC_RCA_HOST_THIRD_PARTY_REBOOT_FAIL, hostName, e.getI18nMessage());
            return DefaultBatchTaskItemResult.builder().batchTaskItemStatus(BatchTaskItemStatus.FAILURE)
                    .msgKey(RcaBusinessKey.RCDC_RCA_HOST_THIRD_PARTY_REBOOT_FAIL).msgArgs(new String[]{hostName, e.getI18nMessage()}).build();
        }
    }

    @Override
    public BatchTaskFinishResult onFinish(int sucCount, int failCount) {
        return buildDefaultFinishResult(sucCount, failCount, RcaBusinessKey.RCDC_RCA_HOST_THIRD_PARTY_REBOOT_BATCH_RESULT);
    }
