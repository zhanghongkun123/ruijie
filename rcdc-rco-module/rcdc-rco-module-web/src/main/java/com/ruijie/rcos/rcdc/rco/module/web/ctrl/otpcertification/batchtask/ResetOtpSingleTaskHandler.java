 ...
 * Create Time: 2021年08月08日
 *
 * @author zhanghongkun
 */
public class ResetOtpSingleTaskHandler extends AbstractSingleTaskHandler {
    ...
    public ResetOtpSingleTaskHandler(UserOtpCertificationAPI userOtpCertificationAPI, BatchTaskItem batchTaskItem, BaseAuditLogAPI auditLogAPI) {
        super(batchTaskItem);
        Assert.notNull(userOtpCertificationAPI, "userOtpCertificationAPI is not null");
    ...
    public BatchTaskFinishResult onFinish(int successCount, int failCount) {
        // 重置单个用户动态口令
        if (successCount == 1) {
            return DefaultBatchTaskFinishResult.builder().msgKey(OtpCertificationBusinessKey.RCDC_OTP_SECRET_KEY_RESET_SUCCESS)
                    .msgArgs(new String[] {userName}).batchTaskStatus(BatchTaskStatus.SUCCESS).build();
        }
        return DefaultBatchTaskFinishResult.builder().msgKey(OtpCertificationBusinessKey.RCDC_OTP_SECRET_KEY_RESET_FAIL)
