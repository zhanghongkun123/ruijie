 ...
 */
@Service
public class ImportQcow2FileHandlerFactory {
    ...
    private class ImportOsFileHandler extends AbstractSingleTaskHandler {
        ...
        private ImportOsFileHandler(BatchTaskItem batchTaskItem, BaseAuditLogAPI auditLogAPI, CbbImportOsFileDTO webRequest) {
            super(batchTaskItem);
        ...
        public BatchTaskFinishResult onFinish(int successCount, int failCount) {
            if (failCount == 0) {
                return DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.SUCCESS)
                        .msgKey(CloudDesktopBusinessKey.RCDC_CLOUDDESKTOP_OS_UPLOAD_TASK_SUCCESS).msgArgs(new String[] {webRequest.getOsFileName()})
                        .build();
            } else {
                ...
                if (this.exception == null) {
                    result = DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.FAILURE)
                            .msgKey(CloudDesktopBusinessKey.RCDC_CLOUDDESKTOP_OS_UPLOAD_TASK_FAIL).msgArgs(new String[] {webRequest.getOsFileName()})
                            .build();
                } else {
                    result = DefaultBatchTaskFinishResult.builder().batchTaskStatus(BatchTaskStatus.FAILURE)
        ...
        public BatchTaskItemResult processItem(BatchTaskItem item) throws BusinessException {
            try {
                checkIQCOW2FileType(webRequest.getOsFileName());
                osFileMgmtAPI.importOsFile(this.webRequest);
                auditLogAPI.recordLog(CloudDesktopBusinessKey.RCDC_CLOUDDESKTOP_OS_UPLOAD_SUCCESS_LOG, webRequest.getOsFileName());
                ...
                auditLogAPI.recordLog(CloudDesktopBusinessKey.RCDC_CLOUDDESKTOP_OS_UPLOAD_FAIL_LOG, e, webRequest.getOsFileName(),
                        e.getI18nMessage());
    ...
    // 校验镜像类型是QCOW2
    private ImageFormat checkIQCOW2FileType(String fileName) throws BusinessException {
        String lowerCaseFileName = fileName.toLowerCase();
        boolean enable = lowerCaseFileName.endsWith(FilePathContants.QCOW2_TYPE);
        if (!enable) {
            // 镜像文件类型不是QCOW2
            throw new BusinessException(CloudDesktopBusinessKey.RCDC_CLOUDDESKTOP_OS_FILETYPE_NOT_QCOW2, new String[0]);
        }
