 ...
 * @author XiaoJiaXin
 */
public class UploadRcdcLoginLogoBatchTaskHandlerTest {


    @Tested
    private UploadRcdcLoginLogoBatchTaskHandler handler;
    ...
    public void testOnFinish() {
        ...
        Assert.assertEquals(batchTaskFinishResult.getMsgKey(), RcoBusinessKey.RCDC_RCO_UPLOAD_RCDC_LOGO_TASK_SUCCESS);
        batchTaskFinishResult = handler.onFinish(0, 1);
        Assert.assertEquals(batchTaskFinishResult.getStatus(), BatchTaskStatus.FAILURE);
        Assert.assertEquals(batchTaskFinishResult.getMsgKey(), RcoBusinessKey.RCDC_RCO_UPLOAD_RCDC_LOGO_TASK_FAIL);
    }
}
