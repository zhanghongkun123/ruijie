...
/**
 * Description: 异步线程处理同步LDAP域用户请求
 * Copyright: Copyright (c) 2023
 * Company: Ruijie Co., Ltd.
 * Create Time: 2023-11-24
 *
 * @author zdc
 */
public class AsyncBatchSyncLdapUserThread extends AbstractAsyncUserMgmtThread {

    private static final Logger LOGGER = LoggerFactory.getLogger(AsyncBatchSyncLdapUserThread.class);
    ...
    private static final ThreadExecutor THREAD_EXECUTOR =
            ThreadExecutors.newBuilder(Constant.BATCH_SYNC_LDAP_USER_THREAD).maxThreadNum(MAX_THREAD_NUM).queueSize(QUEUE_SIZE).build();

    public AsyncBatchSyncLdapUserThread(AsyncTaskEnum action, OpenApiTaskInfoAPI openApiTaskInfoAPI,
    ...
    private void batchTask() {
        ...
        try {
            ...
            LOGGER.error(String.format("等待中断异常，批量同步LDAP域用户超时,任务Id：%s", this.customTaskId), e);
            saveTaskResult(taskItemRequestList, successCount, errIdentityList, syncAdUserList);
        ...
        for (SyncAdUserRequest syncAdUserRequest : syncAdUserList) {
            ...
            THREAD_EXECUTOR.execute(() -> {
                try {
                    ...
                    iacCreateDomainUserRequest.setUserType(IacUserTypeEnum.LDAP);
                    ...
                    LOGGER.error(String.format("同步LDAP域用户异常,用户名:%s", syncAdUserRequest.getUserName()), e);
                    dealErrorIdentity(errIdentityList, syncAdUserRequest, taskItemRequest);
                    //用户同步成功，存在云桌面创建失败，需要合并云桌面失败结果
                    String systemErrorTip = LocaleI18nResolver.resolve(BusinessKey.RCDC_USER_SYNC_LDAP_USER_SYSTEM_ERROR);
    ...
    private void buildTaskResult(SyncAdUserRequest syncAdUserRequest, TaskItemRequest taskItemRequest, IacSyncDomainUserDTO domainUserDTO) {
        ...
        taskItemRequest.setItemResult(LocaleI18nResolver.resolve(BusinessKey.RCDC_USER_SYNC_LDAP_USER_SUCCESS));
    }
    ...
    private void dealBusinessException(TaskItemRequest taskItemRequest, String i18nMessage) {
        ...
        if (StringUtils.hasText(taskItemRequest.getItemResult())) {
            ...
            String describe = LocaleI18nResolver.resolve(BusinessKey.RCDC_USER_SYNC_LDAP_USER_FAIL, i18nMessage);
    ...
    private void createDesktop(IacSyncDomainUserDTO domainUserDTO) throws BusinessException {
        if (Boolean.TRUE.equals(domainUserDTO.getHasCreate())) {
            ...
            if (desktopConfigDTO != null) {
                ...
                if (Objects.nonNull(desktopConfigDTO.getDeskSpecId())) {
    ...
    private String getErrorMsg(List<String> errIdentityList) {
        ...
        if (!errIdentityList.isEmpty()) {
            ...
            int segNum = 3;
            int taskNum = Math.min(errIdentityList.size(), segNum);
            ...
            if (errIdentityList.size() <= segNum) {
                rs = LocaleI18nResolver.resolve(BusinessKey.RCDC_USER_SYNC_LDAP_USER_TASK_FAIL_RESULT, String.valueOf(errorMsg));
            } else {
                rs = LocaleI18nResolver.resolve(BusinessKey.RCDC_USER_SYNC_LDAP_USER_TASK_FAILS_RESULT,
    ...
    private String getTaskResult(AtomicInteger successCount, int total) {
        return LocaleI18nResolver.resolve(BusinessKey.RCDC_USER_SYNC_LDAP_USER_TASK_RESULT,
